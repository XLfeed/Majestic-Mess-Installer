...
public class MouseSensitivitySlider : Entity
{
    public string trackEntityName = "SensSliderTrack";
    public string knobEntityName  = "SensSliderKnob";
    ...
    // Sensitivity range
    private const float SENS_MIN = 0.2f;
    private const float SENS_MAX = 20.0f;
    ...
    public override void OnInit()
    {
        ...
        // Initialize from static value if it was set before, otherwise use default
        float currentSens = ThirdPersonCamera.GlobalSensitivity >= 0f
            ? ThirdPersonCamera.GlobalSensitivity
            : 3.0f;
        sliderValue = (currentSens - SENS_MIN) / (SENS_MAX - SENS_MIN);
    ...
    public override void OnUpdate(float deltaTime)
    {
        ...
        {
            isDragging = false;
            float sens = SENS_MIN + (SENS_MAX - SENS_MIN) * sliderValue;
            Debug.Log($"[MouseSensitivitySlider] Sensitivity: {sens:F1}");
        }
    }
    ...
    private void ApplyMouseX(float mouseUIX)
    {
        ...
        float sens = SENS_MIN + (SENS_MAX - SENS_MIN) * sliderValue;
        ThirdPersonCamera.GlobalSensitivity = sens;
        UpdateKnobPosition();
        UpdateLabel();
    }
    ...
    private void UpdateLabel()
    {
        ...
        {
            float sens = SENS_MIN + (SENS_MAX - SENS_MIN) * sliderValue;
            InternalCalls.UITextComponent_SetText(label.ID, $"{sens:F1}");
        }
    }
}
